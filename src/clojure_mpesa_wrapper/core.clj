...
(defn balance [{:keys [access-token initiator short-code remarks queue-url result-url security-credential]
...
(defn b2b [{:keys [access-token initiator security-credential command-id sender-identifier-type receiver-identifier-type amount
...
;;   occasion -       Optional
(defn b2c [{:keys [access-token initiator-name security-credential command-id amount sender-party receiver-party
...
(defn c2b-reg [{:keys [access-token short-code response-type confirmation-url validation-url]}]
...
;;   OriginatorConversationID -     A unique numeric code generated by the M-Pesa system of the request.
;;   ResponseDescription -          A response message from the M-Pesa system accompanying the response to a request.
(defn c2b-sim [{:keys [access-token short-code command-id amount msisdn bill-ref-number]}]
  {:pre [(and (not= short-code nil) (number? short-code))
         (and (not= command-id nil) (string? command-id))
         (and (not= amount nil) (number? amount))
         (and (not= msisdn nil) (number? msisdn))
         (and (or (and (= command-id "CustomerPayBillOnline")
                       (not= bill-ref-number nil))
                  (and (= command-id "CustomerBuyGoodsOnline")
                       (= bill-ref-number nil))) (string? bill-ref-number))]}
  (let [{:keys [body]}
...
(defn reversal [{:keys [access-token transaction-id amount queue-url result-url short-code remarks occasion initiator
